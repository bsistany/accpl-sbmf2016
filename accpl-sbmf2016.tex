...
\subsection{Productions} \label{sec:productionast}
...
\begin{lstlisting}[basicstyle=\tiny, frame=single, caption={agreement},label={lst:agreementast}]
<agreement> ::= 
...
We started with the encoding of the policy statement for ``First Agreement for Alice and Bob'' earlier but we deferred the definition of the Coq constructs used to encode that agreement. All the definitions needed to encode the agreement for Alice and Bob have now been defined so we now show how that agreement looks like as encodings of \ac{ACCPL} constructs in Coq in the listing~\ref{lst:agreementincoq}. Recall the listing~\ref{lst:agreementast} for the abstract syntax form of the statement.
